...
$(document).mouseup(function (e) {
    let $blockWrap = $(".block-wrap");
    for (let i = 0; i < $blockWrap.length; i++) {
        let $itemEl = $blockWrap[i];
        if ($itemEl !== (e.target) && !$itemEl.contains(e.target)) {
            $itemEl.classList.add("block-inactive")
        }
    }
});
